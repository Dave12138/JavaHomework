...
public class Matrix {
    ...
    double getData(int rol, int col) {
     ...
     * @param value
     */
    void setData(int rol, int col, double value) {
        if (rol < 1 || col < 1 || rol > getRows() || col > getCols()) {
            throw new IllegalIndexException();
        }
     ...
     */
    Matrix transpose(){
        Matrix trans = new Matrix(getCols(), getRows());
        for (int i = 0; i < getRows(); i++) {
            for (int j = 0; j < getCols(); j++) {
    ...
    public boolean equals(Object o) {
        ...
        for (int i = 0; i < getRows(); i++) {
            for (int j = 0; j < getCols(); j++) {
                if(data[i][j]!=matrix.data[i][j]) return false;
            }
        }
        return true;
    }
